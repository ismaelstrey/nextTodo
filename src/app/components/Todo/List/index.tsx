...
const TodoList = ({ todo, type }: Lista) => {
  ...
  const RenderCard = () => <div className="flex w-full h-full rounded-md">
    <ul className="flex flex-col gap-2">
      {filtra().map((l: List) => {
        ...
        return (
          <div key={l.id} className={`flex flex-col shadow-md border-2 border-indigo-500 rounded-b-md hover:animate-pulse hover:cursor-pointer ${colorCard(l.status)}`} onClick={(() => handleCliclkOnCard(l.id))}>
